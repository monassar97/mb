...
export class PageShopComponent implements OnInit, OnDestroy {
    ...
    get hasSidebar(): boolean {
        return (
            ...
            ['grid-3-sidebar', 'grid-4-sidebar'].includes(this.gridLayout)
        );
    }
    ...
    ngOnInit(): void {
        const data$: Observable<PageShopData> = this.route.data as Observable<
            PageShopData
        >;

        const category$: Observable<ShopCategory> = data$.pipe(
                ...
                switchMap((data: PageShopData) =>
                    merge(
                        ...
                        this.page.optionsChange$.pipe(
                            ...
                            switchMap((options) =>
                                this.shop.getProductsList(options)
                            )
                        )
                    )
                ),
